...
# 数值变量的极端值处理
def extreme_values(df,col,bottom=0.25,up=0.75,limit_multiple=1.5):
    # 计算下四分位数和上四分位
    
    Q1 = df[col].quantile(q = bottom)
    Q3 = df[col].quantile(q = up)
    
    # 基于1.5倍的四分位差计算上下须对应的值
    low_whisker = Q1 - limit_multiple*(Q3 - Q1)
    up_whisker = Q3 + limit_multiple*(Q3 - Q1)
    
    # 寻找异常点
    df[col][(df[col] > up_whisker) | (df[col] < low_whisker)] = df[col].mode()
    print(col + ' was filled with mode')
