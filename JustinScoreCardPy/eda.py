...
def plot_num_col(df,col_list,hspace=0.4,wspace=0.4,plt_type=None,plt_size=None,plt_num=None,x=None,y=None):
    ...
    if plt_type=='box':
        for i,col in zip(range(1,plt_num+1,1),col_list):
            plt.subplot(x,y,i)
            plt.title(col)
            sns.boxplot(data=df,x=col,fliersize=5,flierprops = {'markerfacecolor':'cornflowerblue', 'markeredgecolor':'cornflowerblue', 'markersize':4}, )
            plt.xlabel('')
...
#plot_default_cate(df,['order_status_chinese'],'target',hspace=0.4,wspace=0.4,plt_size=(10,10),plt_num=1,x=1,y=1)
...
#plot_default_num(df,['score'],'target',hspace=0.4,wspace=0.4,q=10,plt_size=(10,10),plt_num=1,x=1,y=1)